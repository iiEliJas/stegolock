...
#pragma pack(push, 1)  // Turns off alignment padding
...
#pragma pack(pop)   // Turns alignment padding back on
   ...
   // Embed data into lsbs
    ...
    for (size_t i = 0; i < total_embed_len; i++) {
        for (int bit = 0; bit < BITS_PER_BYTE; bit++) {
            ...
            pixel_data[pixel_idx] = (pixel_data[pixel_idx] & ~1) | bit_val;
            pixel_idx++;
        }
    }
...
unsigned char *extract_data(const char *image_path, size_t *data_len) {
    ...
    for (size_t i = 0; i < sizeof(size_t); i++) {
        for (int bit = 0; bit < BITS_PER_BYTE; bit++) {
            // Get lsb from the current image
            unsigned char bit_val = pixel_data[pixel_idx] & 1;
